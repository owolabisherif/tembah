...
export default function Index() {
    ...
    const handleGetRows = async () => {
        let res = await axios.get<PaginatedType>(route('ad.show') + `?page=${useStoredPage('ad-page')}`);

        if (!res.data.data.length) {
            localStorage.setItem('ad-page', '1');
            res = await axios.get<PaginatedType>(route('ad.show'));
        }
    ...
    const handlePageClick = async ({ selected }: { selected: number }) => {
        const res = await axios.get<PaginatedType>(route('ad.show') + `?page=${selected + 1}`);
        localStorage.setItem('ad-page', (selected + 1).toString());
        setData(res.data);
    };
    ...
    const setData = (payload: PaginatedType) => {
        ...
        setRows(payload.data as AdType[]);
    };

    const handleDelete = async (ad: AdType) => {
        try {
            if (confirm('You are about to perform a delete request. proceed ?')) {
                const res = await axios.delete(route('ad.delete', { ad: ad.id }));
                setType('success');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex items-center justify-end px-3 py-3">
                <Link href={route('ad.create')} className="rounded-sm bg-blue-900 px-3 py-2 text-sm text-white hover:bg-blue-700">
                    CREATE AD
                </Link>
            </div>
            ...
            {Boolean(rows.length) ? (
                <Paginator
                    data={rows}
                    intialPage={useStoredPage('ad-page') - 1}
                    pageCount={pageCount}
                            ...
                            <table className="mb-3 table w-full table-auto">
                                <thead className="">
                                    <tr className="text-brand-slate border-b shadow-xs">
                                        ...
                                        <th className="text-brand-slate px-2 py-4 text-left text-sm">{t('External URL')}</th>
                                        <th className="text-brand-slate px-2 py-4 text-left text-sm">{t('Priority')}</th>
                                        <th className="text-brand-slate px-2 py-4 text-left text-sm">{t('Starts At')}</th>
                                        <th className="text-brand-slate px-2 py-4 text-left text-sm">{t('Ends At')}</th>
                                        ...
                                        >
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm">
                                                <div className="flex items-center gap-x-2">
                                                    ...
                                                    <p className="flex-1 font-bold">{`${item.title}`}</p>
                                                </div>
                                            </td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm whitespace-nowrap">{item.type}</td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm whitespace-nowrap">{item.url}</td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm whitespace-nowrap">{item.priority}</td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm whitespace-nowrap">{item.starts_at}</td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm whitespace-nowrap">{item.ends_at}</td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm">
                                                <Status
