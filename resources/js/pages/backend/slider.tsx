...
import { MessageType } from '@/components/ui/message-box';
...
export default function Index() {
    ...
    const handleGetRows = async () => {
        let res = await axios.get<PaginatedType>(route('slider.show') + `?page=${useStoredPage('slider-page')}`);

        if (!res.data.data.length) {
            localStorage.setItem('slider-page', '1');
            res = await axios.get<PaginatedType>(route('slider.show'));
        }
    ...
    const handlePageClick = async ({ selected }: { selected: number }) => {
        const res = await axios.get<PaginatedType>(route('slider.show') + `?page=${selected + 1}`);
        localStorage.setItem('slider-page', (selected + 1).toString());
        setData(res.data);
    };
    ...
    const setData = (payload: PaginatedType) => {
        ...
        setRows(payload.data as SliderType[]);
    };

    const handleDelete = async (ad: SliderType) => {
        try {
            if (confirm('You are about to perform a delete request. proceed ?')) {
                const res = await axios.delete(route('slider.delete', { ad: ad.id }));
                setType('success');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {Boolean(rows.length) ? (
                <Paginator
                    data={rows}
                    intialPage={useStoredPage('slider-page') - 1}
                    pageCount={pageCount}
                            ...
                            <table className="mb-3 table w-full table-auto">
                                <thead className="">
                                    <tr className="text-brand-slate border-b shadow-xs">
                                        ...
                                        <th className="text-brand-slate px-2 py-4 text-left text-sm">{t('Related Model')}</th>
                                ...
                                <tbody>
                                    {rows.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm whitespace-nowrap">{item.sliderable_type}</td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm">
                                                <Status
