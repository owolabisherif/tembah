...
export default function NewsForm({ newsTypes, authors, leagues, teams, news }: CreateNewsProp) {
    ...
    const { error, value, errorKeys, messages } = useNewsValidator(form);
    ...
    const handleFormPrefill = async () => {
        ...
        if (news.type == 'video') {
            video = await useUrlToImageConverter(news.video_url);
        ...
        const delta = bodyRef.current.clipboard.convert({ html: news.body });
        const deltaAr = bodyRef.current.clipboard.convert({ html: news.body_ar });
    ...
    const submit = async () => {
        ...
        try {
            const res = await axios.post(route('news.store'), form, {
                headers: {
            ...
            <form className="grid h-full grid-cols-12 gap-x-4">
                                ...
                                <div className={cn(form.type == 'text' ? 'col-span-8' : 'col-span-6')}>
                                    ...
                                    <div className={cn('h-40 w-full', form.type !== 'video' && 'hidden')}>
                                        <DropZone
                                            ...
                                            ref={dropVideoZone}
                                        />
                                        {errorKeys.includes('video') && <p className="text-xs text-red-500">{messages['video']}</p>}
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="mt-10">
                            <div className="flex w-full justify-end">
                                <SubmitScheduleDropdown
                                    ...
                                    defaultText={`${news ? 'Update Post' : form.action?.type == 'schedule' ? 'Schedule Post' : 'Post Now'}`}
                ...
                <div className="col-span-4 h-full">
                    <div className="border-brand-gray h-full rounded-sm border p-3 shadow-sm">
                        <div className="mb-5">
                            <h3 className="font-bold">News settings</h3>
                        ...
                        <div className="col-span-12">
                            ...
                            {/* <div className="col-span-12 mb-1">
                                ...
                                <RadSelect
                                    defaultValue={'no'}
                                    onValueChange={(val) => handleCustomChange('is_featured', val === 'yes' ? true : false)}
                            ...
                            <div className="col-span-12 mb-1">
                                <Label htmlFor="showDiscLabel">Add to TOP list ?</Label>
                                <RadSelect
                                    defaultValue={'no'}
                                    onValueChange={(val) => handleCustomChange('in_top', val === 'yes' ? true : false)}
                                    value={form.in_top ? 'yes' : 'no'}
                                    name="showDiscLabel"
                                >
                                    <SelectTrigger className="w-full">
                                        <SelectValue placeholder="Select one" />
