...
export default function Index() {
    ...
    const handlePageClick = async ({ selected }: { selected: number }) => {
        const res = await axios.get<PaginatedType>(route('news.show') + `?page=${selected + 1}`);
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {Boolean(rows.length) ? (
                <Paginator
                    data={rows}
                    intialPage={useStoredPage('news-page') - 1}
                    pageCount={pageCount}
                                        ...
                                        >
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm">
                                                <div className="flex items-center gap-x-2">
                                                    <div className="relative h-8 w-8 overflow-hidden rounded-full">
                                                        <div className="absolute inset-0 flex h-8 w-8 items-center justify-center rounded-full bg-black/35">
                                                            {item.images?.length ? (
                                                                <p className="font-bold text-white">
                                                                    {item.images.length > 1 ? `+ ${item.images.length - 1}` : ''}
                                                                </p>
                                                            ) : (
                                                                ''
                                                            )}
                                            ...
                                            <td className="flex gap-x-2 py-2">
                                                <div className="h-8 w-8">
                                                    <Link
                                                        href={route('news.edit', { slug: item.slug })}
