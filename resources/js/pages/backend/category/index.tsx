...
export default function Index() {
    ...
    const handleGetRows = async () => {
        let res = await axios.get<PaginatedType>(route('category.show') + `?page=${useStoredPage('category-page')}`);

        if (!res.data.data.length) {
            localStorage.setItem('category-page', '1');
            res = await axios.get<PaginatedType>(route('category.show'));
        }
    ...
    const handlePageClick = async ({ selected }: { selected: number }) => {
        const res = await axios.get<PaginatedType>(route('category.show') + `?page=${selected + 1}`);
        localStorage.setItem('category-page', (selected + 1).toString());
        setData(res.data);
    };
    ...
    const handleDelete = async (payload: CatTagType) => {
        try {
            if (confirm('You are about to perform a delete request. proceed ?')) {
                const res = await axios.delete(route('category.delete', { category: payload.id }));
                setType('success');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex items-center justify-end px-3 py-3">
                <Link href={route('category.create')} className="rounded-sm bg-blue-900 px-3 py-2 text-sm text-white hover:bg-blue-700">
                    CREATE CATEGORY
                </Link>
            </div>
            ...
            {Boolean(rows.length) ? (
                <Paginator
                    data={rows}
                    intialPage={useStoredPage('category-page') - 1}
                    pageCount={pageCount}
                            ...
                            <table className="mb-3 table w-full table-auto">
                                <thead className="">
                                    <tr className="text-brand-slate border-b shadow-xs">
                                        ...
                                        <th className="text-brand-slate px-2 py-4 text-left text-sm">{t('Order')}</th>
                                ...
                                <tbody>
                                    {rows.map((item, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm whitespace-nowrap">{item.sort ?? 'None'}</td>
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm">
                                                <Status
