...
export default function Index() {
    const [rows, setRows] = useState<CountryType[]>([]);
    ...
    const handleGetRows = async () => {
        let res = await axios.get<PaginatedType>(route('country.show') + `?page=${useStoredPage('country-page')}`);

        if (!res.data.data.length) {
            localStorage.setItem('country-page', '1');
            res = await axios.get<PaginatedType>(route('country.show'));
        }
    ...
    const handlePageClick = async ({ selected }: { selected: number }) => {
        const res = await axios.get<PaginatedType>(route('country.show') + `?page=${selected + 1}`);
        localStorage.setItem('country-page', (selected + 1).toString());
        setData(res.data);
    };
    ...
    const setData = (payload: PaginatedType) => {
        ...
        setRows(payload.data as CountryType[]);
    };

    const handleDelete = async (payload: CountryType) => {
        try {
            if (confirm('You are about to perform a delete request. proceed ?')) {
                const res = await axios.delete(route('country.delete', { tag: payload.id }));
                setType('success');
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            {Boolean(rows.length) ? (
                <Paginator
                    data={rows}
                    intialPage={useStoredPage('country-page') - 1}
                    pageCount={pageCount}
                                        ...
                                        >
                                            <td className="text-brand-slate px-2 py-1 text-left text-sm">
                                                <div className="flex items-center gap-x-2">
                                                    ...
                                                    <p className="flex-1 font-bold capitalize">{`${item.name} (${item.name_ar})`}</p>
                                                </div>
                                            </td>
                                            ...
                                            <td className="flex gap-x-2 py-2">
                                                <div className="h-8 w-8">
                                                    <Link
                                                        href={route('country.edit', { country: item.id })}
